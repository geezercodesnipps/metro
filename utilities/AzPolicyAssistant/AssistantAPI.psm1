...
function Invoke-AzOpenAIUploadFile {
    ...
    $uploadUri = "$Endpoint/openai/files?api-version=2024-05-01-preview"
...
function New-AzOpenAIAssistant {
    ...
    $toolsArray = @(@{ type = "file_search" }, @{ type = "code_interpreter" })
    ...
    $assistantUri = "$Endpoint/openai/assistants?api-version=2024-05-01-preview"
...
function Start-AzOpenAIAssistantThreadRun {
    ...
    $threadRunUri = "$Endpoint/openai/threads/$ThreadID/runs?api-version=2024-05-01-preview"
    ...
    if (-not $Async) {
        ...
        do {
            ...
            $statusUri = "$Endpoint/openai/threads/$ThreadID/runs/$($runResponse.id)?api-version=2024-05-01-preview"
        ...
        if ($runResult.status -eq "completed") {
            ...
            $messagesUri = "$Endpoint/openai/threads/$ThreadID/messages?api-version=2024-05-01-preview"
...
function Get-AzOpenAIAssistantThreadStatus {
    ...
    $statusUri = "$Endpoint/openai/threads/$ThreadID/runs/$RunID?api-version=2024-05-01-preview"
...
function Start-AzOpenAIAssistantThreadWithMessages {
    ...
    $threadUri = "$Endpoint/openai/threads/runs?api-version=2024-05-01-preview"
    ...
    if (-not $Async) {
        ...
        do {
            $i++
            Write-Host "Checking status $i"
            $statusUri = "$Endpoint/openai/threads/$ThreadID/runs/$($threadResponse.id)?api-version=2024-05-01-preview"
            $runResult = Invoke-RestMethod -uri $statusUri -Headers $authHeader
            $runResult | Select-Object id, assistant_id, thread_id, status, last_error
            Start-Sleep -Seconds 10
        } while ($runResult.status -ne "completed" -and $i -lt 100)
...
function Remove-AzOpenAIAssistants {
    ...
    $assistantsUri = "$Endpoint/openai/assistants?api-version=2024-05-01-preview"
    ...
    foreach ($assistant in $assistants.data) {
        ...
        $deleteUri = "$Endpoint/openai/assistants/$($assistant.id)?api-version=2024-05-01-preview"
...
function Get-AzOpenAIAssistantOutputFiles {
    ...
    $uri = "$Endpoint/openai/files?api-version=2024-05-01-preview"

    # Retrieve the list of files
    $getContent = Invoke-RestMethod -Uri $uri -Headers $authHeader -Method Get

    if (![string]::IsNullOrWhiteSpace($FileId)) {
        # Find the specific file based on file ID and purpose
        $findMyFile = $getContent.data | Where-Object { $_.id -eq "$FileId" -and $_.purpose -eq "assistants_output" }

        # If file is found, proceed to download
        if ($findMyFile -ne $null) {
            $downloadUri = "$Endpoint/openai/files/$FileId/content?api-version=2024-05-01-preview"
            $getFileContent = Invoke-RestMethod -Uri $downloadUri -Headers $authHeader -Method Get

            # Check if LocalFilePath is provided
            if (![string]::IsNullOrWhiteSpace($LocalFilePath)) {
                # Write the file content to a file at the local machine path provided
                $getFileContent | Out-File -FilePath $LocalFilePath -Force -Verbose
            } else {
                Write-Error "Local file path must be provided to download the file."
            }
        } else {
            Write-Error "File not found or does not have purpose 'assistants_output': $FileId"
        }
    } else {
        # Enumerate all files with purpose 'assistants_output'
        $outputFiles = $getContent.data | Where-Object { $_.purpose -eq "assistants_output" }
        if ($outputFiles.Count -gt 0) {
            # Return the output files as PS objects
            $outputFiles | Select-Object -Property object, id, purpose, filename, bytes, created_at, status, status_details
        } else {
            Write-Output "No files found with purpose 'assistants_output'."
        }
    }
}


function Remove-AzOpenAIAssistantOutputFiles {
    param (
        [Parameter(Mandatory = $true)]
        [string]$Endpoint,
    ...
    if (![string]::IsNullOrWhiteSpace($FileId)) {
        ...
        # If file is found, proceed to delete
        if ($findMyFile -ne $null) {
            $deleteUri = "$Endpoint/openai/files/$FileId?api-version=2024-05-01-preview"
            Invoke-RestMethod -Uri $deleteUri -Headers $authHeader -Method Delete

            Write-Output "File with ID $FileId has been deleted."
        } else {
            Write-Error "File not found or does not have purpose 'assistants_output': $FileId"
        }
    } else {
        # Enumerate and delete all files with purpose 'assistants_output'
        $outputFiles = $getContent.data | Where-Object { $_.purpose -eq "assistants_output" }
        if ($outputFiles.Count -gt 0) {
            foreach ($file in $outputFiles) {
                $deleteUri = "$Endpoint/openai/files/$($file.id)?api-version=2024-05-01-preview"
                Invoke-RestMethod -Uri $deleteUri -Headers $authHeader -Method Delete
                Write-Output "File with ID $($file.id) has been deleted."
            }
        } else {
            Write-Output "No files found with purpose 'assistants_output'."
        }
...
function Remove-AzOpenAIAssistantFiles {
    param (
        [Parameter(Mandatory = $true)]
        [string]$Endpoint,
    ...
    if (![string]::IsNullOrWhiteSpace($FileId)) {
        # Find the specific file based on file ID
        $findMyFile = $getContent.data | Where-Object { $_.id -eq "$FileId" }
        ...
        if ($findMyFile -ne $null) {
            ...
            Write-Error "File not found: $FileId"
        }
    } else {
        # Enumerate and delete all files, regardless of purpose
        $outputFiles = $getContent.data
        if ($outputFiles.Count -gt 0) {
            foreach ($file in $outputFiles) {
                $deleteUri = "$Endpoint/openai/files/$($file.id)?api-version=2024-05-01-preview"
                Invoke-RestMethod -Uri $deleteUri -Headers $authHeader -Method Delete
